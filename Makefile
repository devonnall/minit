CFLAGS=-Wall -Wextra -O2 -g -Iinclude -I$(shell brew --prefix openssl)/include
LDFLAGS=-L$(shell brew --prefix openssl)/lib -lssl -lcrypto -lz
CC=clang

SRCS=$(wildcard src/*.c)
TST_SRCS=$(filter-out src/main.c, $(SRCS)) $(wildcard test/*.c)

TST_CFLAGS=-Wall -Wextra -Iinclude -Itest -I$(shell brew --prefix openssl)/include

all: build

build: src/main.c
	$(CC) $(CFLAGS) $(LDFLAGS) -o bin/minit $(SRCS)

test: $(TST_SRCS)
	$(CC) $(CFLAGS) $(LDFLAGS) -o bin/test $(TST_SRCS) 
